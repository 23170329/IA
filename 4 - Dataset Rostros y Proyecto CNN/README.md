...
4. **Entrenamiento** (`04_entrenar_cnn.py`): Transfer Learning con **MobileNetV2** pre-entrenado en ImageNet + Data Augmentation (rotación, zoom, flip, brillo). Dos fases: cabeza con base congelada (15 épocas) + fine-tuning de últimas capas (10 épocas).
5. **Predicción** (`05_test_cnn.py`): Reconocimiento en tiempo real por webcam con umbral de confianza y top-3.
...
scripts/
  ...
  ├── 04_entrenar_cnn.py   # Entrenamiento CNN
  └── 05_test_cnn.py       # Test tiempo real
```
...
# Entrenar CNN
python scripts/04_entrenar_cnn.py

# Probar en tiempo real
python scripts/05_test_cnn.py
```
